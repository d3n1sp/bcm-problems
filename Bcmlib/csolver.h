...
enum Num_Parallel_Masks {
		...
		REGULARIZATION2 = 0x00000040L, //...включение регуляризации матрицы через граничное условие;
...
void CSolver<T>::to_equationHL(int i, int j, T* F, T h)
{
	...
	if (! this->hh[i][1].GetMatrix()) this->hh[i][1].set_matrix(id_norm, NN); T* m = this->hh[i][0][j];
	for (int k = 0; k < NN; k++) m[k] += F[k]*h;
}
